    ...
    public class RegFile : IPolicySource
    {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        else if (data.StartsWith("hex"))
                        {
                            ...
                            {
                                value.Kind = (RegistryValueKind)
                                    int.Parse(
                                        data.Substring(4, indexOfClosingParen - 4),
                                        System.Globalization.NumberStyles.HexNumber
                                    );
                            ...
                            do
                            {
                                var safeLine = (curHexLine ?? string.Empty)
                                    .Trim()
                                    .TrimEnd('\\', ',');
                                var hexBytes = safeLine
                            ...
                            value.Data = PolFile.BytesToObject(
                                allDehexedBytes.ToArray(),
                                value.Kind
                            );
                        }
        ...
        public void Save(TextWriter Writer, Dictionary<string, string>? casePreservation = null)
        {
            ...
            foreach (var key in Keys)
            {
                ...
                if (
                    casePreservation != null
                    && casePreservation.TryGetValue(
                        keyName.ToLowerInvariant() + "\\",
                        out var preservedKey
                    )
                )
                ...
                else
                {
                    ...
                    foreach (var value in key.Values)
                    {
                        ...
                        {
                            string dictKey =
                                keyName.ToLowerInvariant() + "\\" + valueName.ToLowerInvariant();
                        ...
                        else
                        {
                            switch (value.Kind)
                            {
                                case RegistryValueKind.String:
                                {
                                    Writer.Write("\"");
                                    Writer.Write(
                                        EscapeValue(Convert.ToString(value.Data) ?? string.Empty)
                                    );
                                    Writer.WriteLine("\"");
                                    break;
                                }
                                case RegistryValueKind.DWord:
                                {
                                    ...
                                    Writer.WriteLine(
                                        Convert.ToString(dwordVal, 16).PadLeft(8, '0')
                                    );
                                    break;
                                }
                                default:
                                {
                                    ...
                                    switch (value.Kind)
                                    {
                                        case RegistryValueKind.MultiString:
                                            safeData =
                                                value.Data as string[] ?? Array.Empty<string>();
                                            break;
                                    ...
                                    {
                                        Writer.Write(
                                            Convert.ToString(bytes[n], 16).PadLeft(2, '0')
                                        );
                                        Writer.Write(",");
                                        posInRow += 3;
                                        if (posInRow >= 78)
                                    ...
                                    if (bytes.Length > 0)
                                    {
                                        Writer.WriteLine(
                                            Convert
                                                .ToString(bytes[bytes.Length - 1], 16)
                                                .PadLeft(2, '0')
                                        );
                                    }
                                    else
        ...
        private static string CanonicalizeKeyName(string name)
        {
            ...
            // Canonicalize hive
            if (
                parts[0].Equals("HKLM", StringComparison.OrdinalIgnoreCase)
                || parts[0].Equals("HKEY_LOCAL_MACHINE", StringComparison.OrdinalIgnoreCase)
            )
                parts[0] = "HKEY_LOCAL_MACHINE";
            else if (
                parts[0].Equals("HKCU", StringComparison.OrdinalIgnoreCase)
                || parts[0].Equals("HKEY_CURRENT_USER", StringComparison.OrdinalIgnoreCase)
            )
                parts[0] = "HKEY_CURRENT_USER";
            else if (
                parts[0].Equals("HKU", StringComparison.OrdinalIgnoreCase)
                || parts[0].Equals("HKEY_USERS", StringComparison.OrdinalIgnoreCase)
            )
                parts[0] = "HKEY_USERS";
            // Canonicalize common first segments
            if (parts.Length > 1 && parts[1].Equals("software", StringComparison.OrdinalIgnoreCase))
                parts[1] = "SOFTWARE";
            if (parts.Length > 1 && parts[1].Equals("system", StringComparison.OrdinalIgnoreCase))
                parts[1] = "SYSTEM";
            if (parts.Length > 2 && parts[2].Equals("policies", StringComparison.OrdinalIgnoreCase))
                parts[2] = "Policies";
            if (
                parts.Length > 2
                && parts[2].Equals("microsoft", StringComparison.OrdinalIgnoreCase)
            )
                parts[2] = "Microsoft";
            if (parts.Length > 3 && parts[3].Equals("windows", StringComparison.OrdinalIgnoreCase))
                parts[3] = "Windows";
            if (
                parts.Length > 4
                && parts[4].Equals("currentversion", StringComparison.OrdinalIgnoreCase)
            )
                parts[4] = "CurrentVersion";
        ...
        private RegFileKey? GetKey(string Name)
        {
            return Keys.FirstOrDefault(k =>
                k.Name.Equals(Name, StringComparison.InvariantCultureIgnoreCase)
            );
        }
        ...
        {
            return Keys.FirstOrDefault(k =>
                !k.IsDeleter && k.Name.Equals(Name, StringComparison.InvariantCultureIgnoreCase)
            );
        }

        private bool IsSourceKeyAcceptable(string Key)
        {
            return string.IsNullOrEmpty(SourceSubtree)
                || Key.Equals(SourceSubtree, StringComparison.InvariantCultureIgnoreCase)
                || Key.StartsWith(
                    SourceSubtree + "\\",
        ...
        private static string StripHivePrefix(string path)
        {
            ...
            if (TryStrip(Lower, p, "hkey_local_machine\\", out var r))
                return r;
            if (TryStrip(Lower, p, "hkey_current_user\\", out r))
                return r;
            if (TryStrip(Lower, p, "hkey_users\\", out r))
                return r;
            if (TryStrip(Lower, p, "hklm\\", out r))
                return r;
            if (TryStrip(Lower, p, "hkcu\\", out r))
                return r;
            if (TryStrip(Lower, p, "hku\\", out r))
                return r;
            return p;
        }
