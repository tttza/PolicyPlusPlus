...
namespace PolicyPlusCore.Admx
{
    public class AdmxFile
    {
        ...
        public List<AdmxSupportDefinition> SupportedOnDefinitions =
            new List<AdmxSupportDefinition>();
                ...
                switch (child.LocalName ?? "")
                {
                    case "policyNamespaces": // Referenced namespaces and current namespace
                    {
                        foreach (XmlNode policyNamespace in child.ChildNodes)
                        {
                            ...
                            if (
                                !string.IsNullOrEmpty(nsAttr)
                                && policyNamespace.LocalName == "target"
                            )
                        ...
                        foreach (XmlNode supportInfo in child.ChildNodes)
                        {
                                ...
                                foreach (XmlNode supportDef in supportInfo.ChildNodes)
                                {
                                    ...
                                    foreach (XmlNode logicElement in supportDef.ChildNodes)
                                    {
                                        ...
                                        if (canLoad)
                                        {
                                            ...
                                            foreach (
                                                XmlNode conditionElement in logicElement.ChildNodes
                                            )
                                            {
                                                if (conditionElement.LocalName == "reference")
                                                {
                                                    var product = conditionElement
                                                        .Attributes
                                                        ?["ref"]
                                                        ?.Value;
                                                    if (!string.IsNullOrEmpty(product))
                                                        definition.Entries.Add(
                                                            new AdmxSupportEntry()
                                                            {
                                                                ProductID = product,
                                                                IsRange = false,
                                                            }
                                                ...
                                                else if (conditionElement.LocalName == "range")
                                                {
                                                    ...
                                                    entry.ProductID =
                                                        conditionElement.Attributes?["ref"]?.Value
                                                        ?? string.Empty;
                                                    var maxVerAttr = conditionElement
                                                        .Attributes
                                                        ?["maxVersionIndex"]
                                                        ?.Value;
                                                    if (
                                                        !string.IsNullOrEmpty(maxVerAttr)
                                                        && int.TryParse(maxVerAttr, out var maxVer)
                                                    )
                                                        entry.MaxVersion = maxVer;
                                                    var minVerAttr = conditionElement
                                                        .Attributes
                                                        ?["minVersionIndex"]
                                                        ?.Value;
                                                    if (
                                                        !string.IsNullOrEmpty(minVerAttr)
                                                        && int.TryParse(minVerAttr, out var minVer)
                                                    )
                            ...
                            else if (supportInfo.LocalName == "products") // Product definitions
                            {
                                ...
                                )
                                {
                                    ...
                                    {
                                        if (
                                            (subproductElement.LocalName ?? "")
                                            != (ChildTagName ?? "")
                                        )
                                            continue;
                                        var product = new AdmxProduct();
                                        var pname = subproductElement.Attributes?["name"]?.Value;
                                        var pdisp = subproductElement
                                            .Attributes
                                            ?["displayName"]
                                            ?.Value;
                                        if (
                                            string.IsNullOrEmpty(pname)
                                            || string.IsNullOrEmpty(pdisp)
                                        )
                                            continue;
                                        product.ID = pname;
                                        product.DisplayCode = pdisp;
                                        if (Parent is not null)
                                        {
                                            var verStr = subproductElement
                                                .Attributes
                                                ?["versionIndex"]
                                                ?.Value;
                                            if (
                                                !string.IsNullOrEmpty(verStr)
                                                && int.TryParse(
                                                    verStr,
                    ...
                    case "policies": // Policy settings
                    {
                            ...
                            foreach (XmlNode subElement in Node.ChildNodes)
                            {
                                ...
                                else if (subElement.LocalName == "decimal")
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(valStr))
                                        regItem.NumberValue = uint.Parse(
                                            valStr,
                                            CultureInfo.InvariantCulture
                                        );
                                    break;
                        ...
                        ;
                        PolicyRegistryList loadOnOffValList(
                            string OnValueName,
                            string OffValueName,
                            string OnListName,
                            string OffListName,
                        ...
                        foreach (XmlNode polElement in child.ChildNodes)
                        {
                            ...
                            policy.DisplayCode =
                                polElement.Attributes?["displayName"]?.Value ?? string.Empty;
                            policy.RegistryKey =
                                polElement.Attributes?["key"]?.Value ?? string.Empty;
                            string polClass =
                                polElement.Attributes?["class"]?.Value ?? string.Empty;
                            switch (polClass ?? "")
                            {
                                case "Machine":
                                {
                            ...
                            policy.ClientExtension =
                                polElement.AttributeOrNull("clientExtension") ?? string.Empty;
                            policy.RegistryValue =
                                polElement.AttributeOrNull("valueName") ?? string.Empty;
                            policy.AffectedValues = loadOnOffValList(
                                "enabledValue",
                                "disabledValue",
                                "enabledList",
                                "disabledList",
                            ...
                            foreach (XmlNode polInfo in polElement.ChildNodes)
                            {
                                switch (polInfo.LocalName ?? "")
                                {
                                    ...
                                    {
                                        policy.CategoryID =
                                            polInfo.Attributes?["ref"]?.Value ?? string.Empty;
                                        break;
                                    }

                                    case "supportedOn":
                                    {
                                        policy.SupportedCode =
                                            polInfo.Attributes?["ref"]?.Value ?? string.Empty;
                                        break;
                                    }
                                        ...
                                        foreach (XmlNode uiElement in polInfo.ChildNodes)
                                        {
                                            ...
                                            switch (uiElement.LocalName ?? "")
                                            {
                                                case "decimal":
                                                {
                                                    ...
                                                    decimalEntry.Minimum = Convert.ToUInt32(
                                                        uiElement.AttributeOrDefault("minValue", 0),
                                                        CultureInfo.InvariantCulture
                                                    );
                                                    decimalEntry.Maximum = Convert.ToUInt32(
                                                    ...
                                                    decimalEntry.NoOverwrite = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault("soft", false)
                                                    );
                                                    decimalEntry.StoreAsText = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "storeAsText",
                                                            false
                                                        )
                                                ...
                                                case "boolean":
                                                {
                                                    ...
                                                    boolEntry.AffectedRegistry = loadOnOffValList(
                                                        "trueValue",
                                                        "falseValue",
                                                        "trueList",
                                                        "falseList",
                                                ...
                                                case "text":
                                                {
                                                    ...
                                                    textEntry.MaxLength = Convert.ToInt32(
                                                        uiElement.AttributeOrDefault(
                                                            "maxLength",
                                                            255
                                                        ),
                                                    ...
                                                    textEntry.Required = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "required",
                                                            false
                                                        )
                                                    );
                                                    textEntry.RegExpandSz = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "expandable",
                                                            false
                                                        )
                                                    );
                                                    textEntry.NoOverwrite = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault("soft", false)
                                                    );
                                                    entry = textEntry;
                                                    break;
                                                ...
                                                case "list":
                                                {
                                                    ...
                                                    listEntry.NoPurgeOthers = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "additive",
                                                            false
                                                        )
                                                    );
                                                    listEntry.RegExpandSz = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "expandable",
                                                            false
                                                        )
                                                    );
                                                    listEntry.UserProvidesNames = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "explicitValue",
                                                            false
                                                        )
                                                    ...
                                                    listEntry.HasPrefix =
                                                        uiElement.Attributes?["valuePrefix"]
                                                        is object;
                                                    listEntry.RegistryValue =
                                                        uiElement.AttributeOrNull("valuePrefix")
                                                        ?? string.Empty;
                                                    entry = listEntry;
                                                    break;
                                                }
                                                ...
                                                case "enum":
                                                {
                                                    ...
                                                    enumEntry.Required = Convert.ToBoolean(
                                                        uiElement.AttributeOrDefault(
                                                            "required",
                                                            false
                                                        )
                                                    ...
                                                    enumEntry.Items =
                                                        new List<EnumPolicyElementItem>();
                                                    foreach (
                                                        XmlNode itemElement in uiElement.ChildNodes
                                                    )
                                                    {
                                                        if (itemElement.LocalName == "item")
                                                        {
                                                            ...
                                                            enumItem.DisplayCode =
                                                                itemElement
                                                                    ...
                                                                    ?.Value ?? string.Empty;
                                                            foreach (
                                                                XmlNode valElement in itemElement.ChildNodes
                                                            )
                                                            {
                                                                if (valElement.LocalName == "value")
                                                                {
                                                                    enumItem.Value = loadRegItem(
                                                                        valElement
                                                                    );
                                                                }
                                                                else if (
                                                                    valElement.LocalName
                                                                    == "valueList"
                                                                )
                                                                {
                                                                    enumItem.ValueList =
                                                                        loadOneRegList(valElement);
                                                                }
                                                            }
                                            ...
                                            if (entry is object)
                                            {
                                                entry.ClientExtension =
                                                    uiElement.AttributeOrNull("clientExtension")
                                                    ?? string.Empty;
                                                entry.RegistryKey =
                                                    uiElement.AttributeOrNull("key")
                                                    ?? string.Empty;
                                                if (string.IsNullOrEmpty(entry.RegistryValue))
                                                    entry.RegistryValue =
                                                        uiElement.AttributeOrNull("valueName")
                                                        ?? entry.RegistryValue;
