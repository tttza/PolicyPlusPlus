...
namespace PolicyPlusCore.Core
{
    public class PolicyProcessing
    {
        ...
        )
        {
            ...
            if (
                !string.IsNullOrEmpty(rawpol.RegistryKey)
            ...
            )
            {
                ...
                {
                    LogError("PolicyProcessing", "explicit root match eval failed", ex);
                }
            }
            if (rootOnMatched)
                explicitPos++;
            ...
            )
            {
                ...
                {
                    LogError("PolicyProcessing", "checkValList failed", ex);
                }
            }
            ;
        ...
        private static bool ValuePresent(
            PolicyRegistryValue? Value,
        ...
        private static bool ValueListPresent(
            ...
            foreach (var elem in Policy.RawPolicy.Elements)
            {
                ...
                switch (elem.ElementType ?? "")
                {
                    ...
                    case "text":
                    {
                        state.Add(
                            elem.ID,
                            PolicySource.GetValue(elemKey, elem.RegistryValue) ?? string.Empty
                        );
                        break;
                    }
                    ...
                    case "list":
                    {
                        ...
                        if (listElem.UserProvidesNames)
                        {
                            ...
                            foreach (var value in PolicySource.GetValueNames(elemKey))
                                entries.Add(
                                    value,
                                    Convert.ToString(PolicySource.GetValue(elemKey, value))
                                        ?? string.Empty
                                );
                    ...
                    case "multiText":
                    {
                        state.Add(
                            elem.ID,
                            PolicySource.GetValue(elemKey, elem.RegistryValue)
                                ?? Array.Empty<string>()
                        );
                        break;
                    }
        ...
        private static bool GetRegistryListState(
            IPolicySource PolicySource,
        ...
        private static List<RegistryKeyValuePair> WalkPolicyRegistry(
            IPolicySource PolicySource,
            PolicyPlusPolicy Policy,
            bool Forget
        )
                        ...
                        foreach (var elem in rawpol.Elements)
                        {
                            ...
                            switch (elem.ElementType ?? "")
                            {
                                ...
                                case "boolean":
                                {
                                    ...
                                    if (
                                        booleanElem.AffectedRegistry.OffValue is null
                                        && !checkState
                                    )
                                    {
                                ...
                                case "text":
                                {
                                    ...
                                    if (
                                        rawText.Length > textElem.MaxLength
                                        && textElem.MaxLength > 0
                                    )
                                ...
                                case "list":
                                {
                                    ...
                                    if (listElem.UserProvidesNames)
                                    {
                                        ...
                                        if (optionData is List<KeyValuePair<string, string>> kvps)
                                        {
                                            ...
                                            optionData
                                            is IEnumerable<KeyValuePair<string, string>> kvpEnum
                                        )
                                        {
                                    ...
                                    {
                                        List<string> items =
                                            (optionData as IEnumerable<string>)?.ToList()
                                            ?? new List<string>();
                                        int n = 1;
                                        while (n <= items.Count)
                                        {
