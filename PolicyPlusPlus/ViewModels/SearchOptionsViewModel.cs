...
namespace PolicyPlusPlus.ViewModels
{
    public sealed class SearchOptionsViewModel : INotifyPropertyChanged
    {
        ...
        private void Persist()
        {
            ...
            try
            {
                SettingsService.Instance.UpdateSearchOptions(
                    new SearchOptions
                    {
                        InName = _inName,
                        InId = _inId,
        ...
        public bool InRegistryKey
        {
            get => _inRegistryKey;
            set
            {
                if (value == _inRegistryKey)
                    return;
                _inRegistryKey = value;
                Notify();
                Persist();
            }
        }
        public bool InRegistryValue
        {
            get => _inRegistryValue;
            set
            {
                if (value == _inRegistryValue)
                    return;
                _inRegistryValue = value;
                Notify();
                Persist();
            }
        }
        public bool InDescription
        {
            ...
            set
            {
                if (value == _inDescription)
                    return;
                _inDescription = value;
                Notify();
                Persist();
            }
        }
