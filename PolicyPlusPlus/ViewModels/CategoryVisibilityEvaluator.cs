...
namespace PolicyPlusPlus.ViewModels
{
    ...
    {
        public static bool IsCategoryVisible(
            PolicyPlusCategory cat,
            IEnumerable<PolicyPlusPolicy> allPolicies,
            AdmxPolicySection appliesFilter,
            bool configuredOnly,
        ...
        )
        {
            ...
            var pending =
                Services.PendingChangesService.Instance.Pending?.ToList()
                ?? new List<Services.PendingChange>();

            foreach (var p in seq)
            {
                // Check pending first
                bool hasPendingUser = pending.Any(pc =>
                    string.Equals(pc.PolicyId, p.UniqueID, StringComparison.OrdinalIgnoreCase)
                    && string.Equals(pc.Scope, "User", StringComparison.OrdinalIgnoreCase)
                    && (
                        pc.DesiredState == PolicyState.Enabled
                        || pc.DesiredState == PolicyState.Disabled
                    )
                );
                bool hasPendingComp = pending.Any(pc =>
                    string.Equals(pc.PolicyId, p.UniqueID, StringComparison.OrdinalIgnoreCase)
                ...
                try
                {
                    var comp =
                        compSource != null
                            ? PolicyProcessing.GetPolicyState(compSource, p)
                            : PolicyState.Unknown;
                    var user =
                        userSource != null
                            ? PolicyProcessing.GetPolicyState(userSource, p)
                            : PolicyState.Unknown;
                    if (
                        comp == PolicyState.Enabled
                        || comp == PolicyState.Disabled
                        || user == PolicyState.Enabled
                        || user == PolicyState.Disabled
                    )
                        return true;
                }
                catch { }
