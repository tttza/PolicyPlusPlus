...
namespace PolicyPlusPlus.ViewModels
{
    ...
    public sealed class OptionElementVM : INotifyPropertyChanged
    {
        ...
        private int _userEnumIndex = -1;
        public int UserEnumIndex
        {
            get => _userEnumIndex;
            set
            {
                if (_userEnumIndex != value)
                {
                    _userEnumIndex = value;
                    OnChanged(nameof(UserEnumIndex));
                    if (Parent.UserState == QuickEditState.Enabled)
                        Parent.QueuePending("User");
                }
            }
        }
        private int _computerEnumIndex = -1;
        public int ComputerEnumIndex
        {
            get => _computerEnumIndex;
            set
            {
                if (_computerEnumIndex != value)
                {
                    _computerEnumIndex = value;
                    OnChanged(nameof(ComputerEnumIndex));
                    if (Parent.ComputerState == QuickEditState.Enabled)
                        Parent.QueuePending("Computer");
                }
            }
        }

        private bool _userBool;
        public bool UserBool
        {
            ...
            set
            {
                ...
                {
                    _userBool = value;
                    OnChanged(nameof(UserBool));
                    if (Parent.UserState == QuickEditState.Enabled)
                        Parent.QueuePending("User");
                }
            }
        }
        private bool _computerBool;
        public bool ComputerBool
        {
            get => _computerBool;
            set
            {
                if (_computerBool != value)
                {
                    _computerBool = value;
                    OnChanged(nameof(ComputerBool));
                    if (Parent.ComputerState == QuickEditState.Enabled)
                        Parent.QueuePending("Computer");
                }
            }
        ...
        public string UserText
        {
            ...
            set
            {
                ...
                {
                    _userText = value;
                    OnChanged(nameof(UserText));
                    if (Parent.UserState == QuickEditState.Enabled)
                        Parent.QueuePending("User");
                }
            }
        }
        private string _computerText = string.Empty;
        public string ComputerText
        {
            get => _computerText;
            set
            {
                if (_computerText != value)
                {
                    _computerText = value;
                    OnChanged(nameof(ComputerText));
                    if (Parent.ComputerState == QuickEditState.Enabled)
                        Parent.QueuePending("Computer");
                }
            }
        ...
        public uint? UserNumber
        {
            ...
            set
            {
                ...
                {
                    _userNumber = value;
                    OnChanged(nameof(UserNumber));
                    OnChanged(nameof(UserNumberDouble));
                    if (Parent.UserState == QuickEditState.Enabled)
                        Parent.QueuePending("User");
                }
            }
        }
        private uint? _computerNumber;
        public uint? ComputerNumber
        {
            ...
            set
            {
                if (_computerNumber != value)
                {
                    ...
                    OnChanged(nameof(ComputerNumber));
                    OnChanged(nameof(ComputerNumberDouble));
                    if (Parent.ComputerState == QuickEditState.Enabled)
                        Parent.QueuePending("Computer");
                }
            }
        ...
        {
            get => _userNumber.HasValue ? _userNumber.Value : double.NaN;
            set
            {
                uint v = double.IsNaN(value) ? 0u : (uint)Math.Max(0, Math.Round(value));
                if (_userNumber != v)
                {
                    _userNumber = v;
                    OnChanged(nameof(UserNumber));
        ...
        public double ComputerNumberDouble
        {
            get => _computerNumber.HasValue ? _computerNumber.Value : double.NaN;
            set
            {
                uint v = double.IsNaN(value) ? 0u : (uint)Math.Max(0, Math.Round(value));
                if (_computerNumber != v)
                {
                    _computerNumber = v;
                    OnChanged(nameof(ComputerNumber));
                    OnChanged(nameof(ComputerNumberDouble));
        ...
        public List<KeyValuePair<string, string>> ComputerNamedListItems { get; private set; } =
            new();
        public string UserListSummary =>
            Type == OptionElementType.List
                ? (
                    ProvidesNames
                        ? $"Edit... ({UserNamedListItems.Count})"
                        : $"Edit... ({UserListItems.Count})"
                )
                : string.Empty;
        public string ComputerListSummary =>
            Type == OptionElementType.List
                ? (
                    ProvidesNames
                        ? $"Edit... ({ComputerNamedListItems.Count})"
                        : $"Edit... ({ComputerListItems.Count})"
                )
                : string.Empty;
        ...
        public string UserMultiTextSummary =>
            Type == OptionElementType.MultiText
                ? $"Edit... ({UserMultiTextItems.Count})"
                : string.Empty;
        public string ComputerMultiTextSummary =>
            Type == OptionElementType.MultiText
                ? $"Edit... ({ComputerMultiTextItems.Count})"
                : string.Empty;
        ...
        )
        {
            ...
            ProvidesNames = providesNames;
            if (type == OptionElementType.Decimal)
            {
                DecimalMin = decMin;
                DecimalMax = decMax;
                DecimalDefault = decDefault;
                _defaultDecimal = decDefault;
                _decimalIncrement = decIncrement;
            }
            ...
            if (element is TextPolicyElement tpe && tpe.Required)
                IsRequired = true;
        }

        public void ReplaceList(bool isUser, List<string> items)
        {
            if (Type != OptionElementType.List || ProvidesNames)
                return;
            if (isUser)
                UserListItems = items;
            else
                ComputerListItems = items;
            OnChanged(isUser ? nameof(UserListSummary) : nameof(ComputerListSummary));
            if (isUser && Parent.UserState == QuickEditState.Enabled)
                Parent.QueuePending("User");
            if (!isUser && Parent.ComputerState == QuickEditState.Enabled)
                Parent.QueuePending("Computer");
        }
        ...
        public void ReplaceNamedList(bool isUser, List<KeyValuePair<string, string>> items)
        {
            if (Type != OptionElementType.List || !ProvidesNames)
                return;
            if (isUser)
                UserNamedListItems = items;
            else
                ComputerNamedListItems = items;
            OnChanged(isUser ? nameof(UserListSummary) : nameof(ComputerListSummary));
            if (isUser && Parent.UserState == QuickEditState.Enabled)
                Parent.QueuePending("User");
            if (!isUser && Parent.ComputerState == QuickEditState.Enabled)
        ...
        {
            if (Type != OptionElementType.MultiText)
                return;
            if (isUser)
                UserMultiTextItems = items;
            else
                ComputerMultiTextItems = items;
        ...
        internal object? GetUserValue()
        {
            return Type switch
            {
                ...
                OptionElementType.List => ProvidesNames
                    ? UserNamedListItems.ToList()
                    : UserListItems.ToList(),
        ...
        internal object? GetComputerValue()
        {
            return Type switch
            {
                ...
                OptionElementType.List => ProvidesNames
                    ? ComputerNamedListItems.ToList()
                    : ComputerListItems.ToList(),
        ...
        internal void LoadFrom(Dictionary<string, object> opts, bool isUser)
        {
            try
            {
                if (!opts.TryGetValue(Id, out var v))
                    return;
                switch (Type)
                {
                    case OptionElementType.Enum:
                        if (v is int ei)
                        {
                            if (isUser)
                                ...
                                _computerEnumIndex = ei;
                            OnChanged(isUser ? nameof(UserEnumIndex) : nameof(ComputerEnumIndex));
                        }
                        break;
                    case OptionElementType.Boolean:
                        if (v is bool b)
                        {
                            if (isUser)
                                ...
                                _computerBool = b;
                            OnChanged(isUser ? nameof(UserBool) : nameof(ComputerBool));
                        }
                        break;
                    case OptionElementType.Text:
                        if (v is string s)
                        {
                            if (isUser)
                                ...
                                _computerText = s;
                            OnChanged(isUser ? nameof(UserText) : nameof(ComputerText));
                        }
                        break;
                    case OptionElementType.Decimal:
                        ...
                        else if (v is int di && di >= 0)
                        {
                            if (isUser)
                                _userNumber = (uint)di;
                            else
                                _computerNumber = (uint)di;
                        }
                    ...
                    case OptionElementType.List:
                        if (ProvidesNames)
                        {
                            ...
                            {
                                var list = dict.Select(k => new KeyValuePair<string, string>(
                                        k.Key,
                                        k.Value
                                    ))
                                    .ToList();
                                if (isUser)
                                    UserNamedListItems = list;
                                else
                                    ComputerNamedListItems = list;
                                OnChanged(
                                    isUser ? nameof(UserListSummary) : nameof(ComputerListSummary)
                                );
                            }
                        ...
                        else
                        {
                            ...
                            {
                                if (isUser)
                                    UserListItems = ll.ToList();
                                else
                                    ComputerListItems = ll.ToList();
                                OnChanged(
                                    isUser ? nameof(UserListSummary) : nameof(ComputerListSummary)
                                );
                            }
                    ...
                    case OptionElementType.MultiText:
                        ...
                        {
                            if (isUser)
                                UserMultiTextItems = ml.ToList();
                            else
                                ComputerMultiTextItems = ml.ToList();
                            OnChanged(
                                isUser
                                    ? nameof(UserMultiTextSummary)
                                    : nameof(ComputerMultiTextSummary)
                            );
                        }
                        else if (v is IEnumerable<string> en2)
                        {
                            var list2 = en2.ToList();
                            if (isUser)
                                UserMultiTextItems = list2;
                            else
                                ComputerMultiTextItems = list2;
                            OnChanged(
                                isUser
                                    ? nameof(UserMultiTextSummary)
                                    : nameof(ComputerMultiTextSummary)
        ...
        internal void InitializeEnumChoices(
            EnumPolicyElement ee,
            Func<string, AdmxFile, string> resolve,
            AdmxFile definedIn
        )
        {
            try
            {
                ...
                foreach (var it in ee.Items)
                {
                    ...
                    try
                    {
                        txt = resolve(it.DisplayCode, definedIn);
                    }
                    catch
                    {
                        txt = it.DisplayCode;
        ...
        internal void EnsureDefaultsAfterLoad()
        {
            ...
            {
                if (_userEnumIndex == -1 && Choices.Count > 0)
                    _userEnumIndex = 0;
                if (_computerEnumIndex == -1 && Choices.Count > 0)
                    _computerEnumIndex = 0;
                OnChanged(nameof(UserEnumIndex));
                OnChanged(nameof(ComputerEnumIndex));
            }
            if (Type == OptionElementType.Decimal)
            {
                if (!_userNumber.HasValue)
                    _userNumber = _defaultDecimal ?? DecimalDefault;
                OnChanged(nameof(UserNumber));
                OnChanged(nameof(UserNumberDouble));
                if (!_computerNumber.HasValue)
                    _computerNumber = _defaultDecimal ?? DecimalDefault;
                OnChanged(nameof(ComputerNumber));
                OnChanged(nameof(ComputerNumberDouble));
            }
            if (Type == OptionElementType.Boolean)
            {
                if (_defaultBool.HasValue)
                {
                    if (!_userBool)
                    {
                        _userBool = _defaultBool.Value;
                        OnChanged(nameof(UserBool));
                    }
                    if (!_computerBool)
                    {
                        _computerBool = _defaultBool.Value;
                        OnChanged(nameof(ComputerBool));
                    }
                }
            }
            if (Type == OptionElementType.Text)
            {
                if (!string.IsNullOrEmpty(_defaultText))
                {
                    ...
                    {
                        _userText = _defaultText;
                        OnChanged(nameof(UserText));
                    }
                    if (string.IsNullOrEmpty(_computerText))
                    {
                        _computerText = _defaultText;
                        OnChanged(nameof(ComputerText));
                    }
                }
            }
