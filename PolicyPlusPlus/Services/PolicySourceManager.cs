...
namespace PolicyPlusPlus.Services
{
    ...
    public interface IPolicySourceManager
    {
        ...
        void Refresh();
        System.Threading.Tasks.Task<(bool ok, string? error)> ApplyPendingAsync(
            AdmxBundle bundle,
            PendingChange[] items,
    ...
    public readonly struct PolicySourceDescriptor
    {
        ...
        public static PolicySourceDescriptor LocalGpo() =>
            new(PolicySourceMode.LocalGpo, null, null);

        public static PolicySourceDescriptor TempPol() => new(PolicySourceMode.TempPol, null, null);

        public static PolicySourceDescriptor Custom(string compPath, string userPath) =>
            new(PolicySourceMode.CustomPol, compPath, userPath);
    }
    ...
    internal sealed class LocalGpoStrategy : IPolicySourceStrategy
    {
        public bool Activate(PolicySourceManager mgr, PolicySourceDescriptor d)
        {
            try
            {
                ...
                mgr.UserSource = new PolicyLoader(
            ...
            {
                Log.Warn("PolicySourceMgr", "LocalGpoStrategy", ex);
                return false;
            }
        }
    }
    ...
    internal sealed class TempPolStrategy : IPolicySourceStrategy
    {
        public bool Activate(PolicySourceManager mgr, PolicySourceDescriptor d)
        {
            ...
            {
                mgr.EnsureTempPaths();
                var compLoader = new PolicyLoader(
                    PolicyLoaderSource.PolFile,
                    mgr.TempCompPath ?? string.Empty,
                    false
                );
                var userLoader = new PolicyLoader(
                    PolicyLoaderSource.PolFile,
                    mgr.TempUserPath ?? string.Empty,
                    true
                );
            ...
            {
                Log.Warn("PolicySourceMgr", "TempPolStrategy", ex);
                return false;
            }
        }
    }
    ...
    internal sealed class CustomPolStrategy : IPolicySourceStrategy
    {
        public bool Activate(PolicySourceManager mgr, PolicySourceDescriptor d)
        {
            if (string.IsNullOrWhiteSpace(d.ComputerPath) || string.IsNullOrWhiteSpace(d.UserPath))
                return false;
    ...
    internal sealed class PolicySourceManager : IPolicySourceManager
    {
        ...
        {
            if (!_strategies.TryGetValue(descriptor.Mode, out var strategy))
                return false;
        ...
        internal void EnsureTempPaths()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(TempCompPath))
                {
                    ...
                    if (!File.Exists(TempCompPath))
                        new PolFile().Save(TempCompPath);
                }
                if (string.IsNullOrEmpty(TempUserPath))
                {
                    TempUserPath = Path.Combine(baseDir, "user.pol");
                    if (!File.Exists(TempUserPath))
                        new PolFile().Save(TempUserPath);
                }
            }
            catch (Exception ex)
            {
                Log.Warn("PolicySourceMgr", "EnsureTempPaths", ex);
            }
        }
        ...
        private static void EnsurePolFileExists(string? path)
        {
            ...
            {
                Log.Warn("PolicySourceMgr", "EnsurePolFileExists", ex);
            }
        }
        ...
        public bool SwitchCustomPolFlexible(string? compPath, string? userPath, bool allowSingle)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(compPath) && string.IsNullOrWhiteSpace(userPath))
                    return false;
                string? finalComp = compPath;
                string? finalUser = userPath;
                if (
                    (string.IsNullOrWhiteSpace(finalComp) || string.IsNullOrWhiteSpace(finalUser))
                    && allowSingle
                )
                {
                    var baseDir = Path.Combine(Path.GetTempPath(), "PolicyPlus");
                    Directory.CreateDirectory(baseDir);
                    if (string.IsNullOrWhiteSpace(finalComp))
                    {
                        finalComp = Path.Combine(baseDir, "_empty_machine.pol");
                        EnsurePolFileExists(finalComp);
                    }
                    if (string.IsNullOrWhiteSpace(finalUser))
                    {
                        finalUser = Path.Combine(baseDir, "_empty_user.pol");
                        EnsurePolFileExists(finalUser);
                    }
                }
                if (string.IsNullOrWhiteSpace(finalComp) || string.IsNullOrWhiteSpace(finalUser))
                    return false;
                CustomCompPath = finalComp;
                CustomUserPath = finalUser;
                EnsurePolFileExists(CustomCompPath);
                EnsurePolFileExists(CustomUserPath);
                var compLoader = new PolicyLoader(
                    PolicyLoaderSource.PolFile,
                    CustomCompPath,
                    false
            ...
            {
                Log.Warn("PolicySourceMgr", "SwitchCustomPolFlexible", ex);
                return false;
            }
        }
        ...
        public void Refresh()
        {
            if (Mode == PolicySourceMode.LocalGpo)
                Switch(PolicySourceDescriptor.LocalGpo());
            else if (Mode == PolicySourceMode.TempPol)
                Switch(PolicySourceDescriptor.TempPol());
            else if (
                Mode == PolicySourceMode.CustomPol
                && !string.IsNullOrEmpty(CustomCompPath)
                && !string.IsNullOrEmpty(CustomUserPath)
            )
                SwitchCustomPolFlexible(CustomCompPath, CustomUserPath, false);
        }

        public async System.Threading.Tasks.Task<(bool ok, string? error)> ApplyPendingAsync(
            AdmxBundle bundle,
            PendingChange[] items,
            IElevationService elevation
        )
        {
            ...
            if (Mode == PolicySourceMode.LocalGpo)
            {
                var saveResult = await SaveChangesCoordinator.SaveAsync(
                    bundle,
                    items,
                    elevation,
            ...
            if (compPol == null || userPol == null)
                return (false, "Current sources not writable POL files");
            foreach (var c in items)
            {
                ...
                void apply(IPolicySource src)
                {
                    ...
                    if (
                        c.DesiredState == PolicyState.Enabled
                        || c.DesiredState == PolicyState.Disabled
                    )
                ...
                if (string.Equals(c.Scope, "Both", StringComparison.OrdinalIgnoreCase))
                {
                    apply(compPol);
                    apply(userPol);
                }
                else if (string.Equals(c.Scope, "User", StringComparison.OrdinalIgnoreCase))
                    apply(userPol);
                else
                    apply(compPol);
            }
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(CustomCompPath))
                        compPol.Save(CustomCompPath);
                    if (!string.IsNullOrEmpty(CustomUserPath))
                        userPol.Save(CustomUserPath);
                }
                else if (Mode == PolicySourceMode.TempPol)
                {
                    if (!string.IsNullOrEmpty(TempCompPath))
                        compPol.Save(TempCompPath);
                    if (!string.IsNullOrEmpty(TempUserPath))
                        userPol.Save(TempUserPath);
                }
            }
            catch (Exception ex)
            {
                Log.Warn("PolicySourceMgr", "Persist POL failed", ex);
