...
namespace PolicyPlusPlus.Services
{
    ...
    {
        public PolicySourceContext(IPolicySource comp, IPolicySource user, PolicySourceMode mode, bool fallbackUsed)
        { Comp = comp; User = user; Mode = mode; FallbackUsed = fallbackUsed; }
    ...
    internal static class PolicySourceAccessor
    {
        ...
        public static PolicySourceContext Acquire()
        {
            ...
            try
            {
                if (mgr.Mode == PolicySourceMode.CustomPol || mgr.Mode == PolicySourceMode.TempPol)
                {
                    if (mgr.CompSource == null || mgr.UserSource == null)
                    {
                        ...
                        Log.Warn("PolicySource", "Sources null in mode=" + mgr.Mode + "; re-switching");
                        if (mgr.Mode == PolicySourceMode.CustomPol && !string.IsNullOrEmpty(mgr.CustomCompPath) && !string.IsNullOrEmpty(mgr.CustomUserPath))
                            mgr.SwitchCustomPolFlexible(mgr.CustomCompPath, mgr.CustomUserPath, allowSingle: true);
            ...
            if (comp == null || user == null)
            {
                ...
                var tempComp = new PolicyLoader(PolicyLoaderSource.PolFile, System.IO.Path.GetTempFileName(), false).OpenSource();
                var tempUser = new PolicyLoader(PolicyLoaderSource.PolFile, System.IO.Path.GetTempFileName(), true).OpenSource();
                Log.Warn("PolicySource", "Created inert fallback sources; application state inconsistent");
