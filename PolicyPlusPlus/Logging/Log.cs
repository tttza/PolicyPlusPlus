...
namespace PolicyPlusPlus.Logging
{
    ...
    public sealed class DebugLogSink : ILogSink
    {
        public void Log(DebugLevel level, string area, string message, Exception? ex = null)
        {
            try
            {
                Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] {level,-5} {area}: {message}{(ex != null ? " :: " + ex.GetType().Name + " - " + ex.Message : string.Empty)}");
    ...
    public static class Log
    {
        ...
        private static void Emit(DebugLevel level, string area, string msg, Exception? ex = null)
        {
            ...
            try { Sink.Log(level, area, msg, ex); } catch { /* swallow */ }
        }
        ...
        {
            if (opts == null || opts.Count == 0) return "(none)";
        ...
        private static string FormatValue(object? value)
        {
            ...
            if (value is string s) return Truncate(s.Replace('\n', ' ').Replace('\r', ' '));
            ...
            {
                int c = 0; var sb = new StringBuilder(); sb.Append('[');
                foreach (var e in en)
                {
                    if (c > 0) sb.Append('|');
