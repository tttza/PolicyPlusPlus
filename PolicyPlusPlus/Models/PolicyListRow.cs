...
namespace PolicyPlusPlus.Models
{
    public sealed class PolicyListRow : INotifyPropertyChanged
    {
        ...
        private bool _userConfigured;
        public bool UserConfigured
        {
            get => _userConfigured;
            private set
            {
                if (_userConfigured != value)
                {
                    _userConfigured = value;
                    OnPropertyChanged(nameof(UserConfigured));
                }
            }
        }
        private bool _computerConfigured;
        public bool ComputerConfigured
        {
            get => _computerConfigured;
            private set
            {
                if (_computerConfigured != value)
                {
                    _computerConfigured = value;
                    OnPropertyChanged(nameof(ComputerConfigured));
                }
            }
        }
        ...
        public bool UserDisabled
        {
            ...
            private set
            {
                if (_userDisabled != value)
                {
                    _userDisabled = value;
                    OnPropertyChanged(nameof(UserDisabled));
                }
            }
        }
        private bool _computerEnabled;
        public bool ComputerEnabled
        {
            get => _computerEnabled;
            private set
            {
                if (_computerEnabled != value)
                {
                    _computerEnabled = value;
                    OnPropertyChanged(nameof(ComputerEnabled));
                }
            }
        }
        private bool _computerDisabled;
        public bool ComputerDisabled
        {
            get => _computerDisabled;
            private set
            {
                if (_computerDisabled != value)
                {
                    _computerDisabled = value;
                    OnPropertyChanged(nameof(ComputerDisabled));
                }
            }
        }

        private string _userGlyph = string.Empty;
        public string UserGlyph
        {
            get => _userGlyph;
            private set
            {
        ...
        private string _computerGlyph = string.Empty;
        public string ComputerGlyph
        {
            get => _computerGlyph;
            private set
            {
                if (_computerGlyph != value)
                {
        ...
        public Brush? UserBrush
        {
            ...
            private set
            {
                if (!ReferenceEquals(_userBrush, value))
                {
                    _userBrush = value;
        ...
        private Brush? _computerBrush;
        public Brush? ComputerBrush
        {
            get => _computerBrush;
            private set
            {
                if (!ReferenceEquals(_computerBrush, value))
                {
                    _computerBrush = value;
                    OnPropertyChanged(nameof(ComputerBrush));
                }
            }
        }

        private bool _userPending;
        public bool UserPending
        {
            get => _userPending;
            private set
            {
                if (_userPending != value)
                {
                    _userPending = value;
                    OnPropertyChanged(nameof(UserPending));
                }
            }
        }
        private bool _computerPending;
        public bool ComputerPending
        {
            get => _computerPending;
            private set
            {
                if (_computerPending != value)
                {
                    _computerPending = value;
                    OnPropertyChanged(nameof(ComputerPending));
                }
            }
        }
        ...
        public string UserStateText
        {
            ...
            private set
            {
                if (_userStateText != value)
                {
                    _userStateText = value;
                    OnPropertyChanged(nameof(UserStateText));
                }
            }
        }
        private string _computerStateText = string.Empty;
        public string ComputerStateText
        {
            get => _computerStateText;
            private set
            {
                if (_computerStateText != value)
                {
                    _computerStateText = value;
                    OnPropertyChanged(nameof(ComputerStateText));
                }
            }
        }
        ...
        private PolicyListRow(
            ...
            IPolicySource? comp,
            IPolicySource? user,
            IEnumerable<PolicyPlusPolicy>? variants,
            string uniqueId,
            string categoryName,
        ...
        public static PolicyListRow FromPolicy(
            PolicyPlusPolicy p,
            IPolicySource? comp,
            IPolicySource? user
        )
        ...
        public static PolicyListRow FromGroup(
        ...
        public void RefreshStateFromSourcesAndPending(
            IPolicySource? compUpdate,
            IPolicySource? userUpdate
        )
        {
            if (compUpdate != null)
                _comp = compUpdate;
            ...
            if (_isGroup)
            {
                bool anyUserConfigured = false,
                    anyCompConfigured = false;
                bool anyUserEnabled = false,
                    anyUserDisabled = false;
                bool anyCompEnabled = false,
                    anyCompDisabled = false;
                bool anyUserPending = false,
                    anyCompPending = false;
                ...
                if (_variants != null)
                {
                    ...
                    {
                        if (
                            v.RawPolicy.Section == AdmxPolicySection.User
                            || v.RawPolicy.Section == AdmxPolicySection.Both
                        )
                        {
                            bool cfg = false,
                                en = false,
                                dis = false,
                                pend = false;
                            var pendUser = pending.FirstOrDefault(pc =>
                                pc.PolicyId == v.UniqueID && pc.Scope == "User"
                            );
                            if (pendUser != null)
                            {
                                en = pendUser.DesiredState == PolicyState.Enabled;
                                dis = pendUser.DesiredState == PolicyState.Disabled;
                                cfg = en || dis;
                                pend = cfg;
                            }
                            else if (_user != null)
                            {
                                try
                                {
                                    ...
                                    en = st == PolicyState.Enabled;
                                    dis = st == PolicyState.Disabled;
                                }
                                catch { }
                            }
                        ...
                        if (
                            v.RawPolicy.Section == AdmxPolicySection.Machine
                            || v.RawPolicy.Section == AdmxPolicySection.Both
                        )
                        {
                            bool cfg = false,
                                ...
                                dis = false,
                                pend = false;
                            var pendComp = pending.FirstOrDefault(pc =>
                                pc.PolicyId == v.UniqueID && pc.Scope == "Computer"
                            );
                            if (pendComp != null)
                            {
                                en = pendComp.DesiredState == PolicyState.Enabled;
                                dis = pendComp.DesiredState == PolicyState.Disabled;
                                cfg = en || dis;
                                pend = cfg;
                            }
                            else if (_comp != null)
                ...
                UserConfigured = anyUserConfigured;
                ComputerConfigured = anyCompConfigured;
                UserEnabled = anyUserEnabled;
                UserDisabled = anyUserDisabled;
                ComputerEnabled = anyCompEnabled;
                ComputerDisabled = anyCompDisabled;

                UserGlyph = anyUserEnabled ? "\uE73E" : (anyUserDisabled ? "\uE711" : string.Empty);
                ComputerGlyph = anyCompEnabled
                    ? "\uE73E"
                    : (anyCompDisabled ? "\uE711" : string.Empty);
            ...
            else if (Policy != null)
            {
                ...
                try
                {
                    if (_user != null)
                    {
                        ...
                        userEn = st == PolicyState.Enabled;
                        userDis = st == PolicyState.Disabled;
                    }
                    if (_comp != null)
                    {
                        var st = PolicyProcessing.GetPolicyState(_comp, Policy);
                        compCfg = st == PolicyState.Enabled || st == PolicyState.Disabled;
                        compEn = st == PolicyState.Enabled;
                        compDis = st == PolicyState.Disabled;
                    }
                }
                catch { }
                ...
                try
                {
                    ...
                    var pendUser = pend.FirstOrDefault(pc =>
                        pc.PolicyId == Policy.UniqueID && pc.Scope == "User"
                    );
                    if (pendUser != null)
                    {
                        userCfg =
                            pendUser.DesiredState == PolicyState.Enabled
                            || pendUser.DesiredState == PolicyState.Disabled;
                        userEn = pendUser.DesiredState == PolicyState.Enabled;
                        userDis = pendUser.DesiredState == PolicyState.Disabled;
                        userPend = userCfg;
                    }
                    var pendComp = pend.FirstOrDefault(pc =>
                        pc.PolicyId == Policy.UniqueID && pc.Scope == "Computer"
                    );
                    if (pendComp != null)
                    {
                        compCfg =
                            pendComp.DesiredState == PolicyState.Enabled
                            || pendComp.DesiredState == PolicyState.Disabled;
                        compEn = pendComp.DesiredState == PolicyState.Enabled;
                        compDis = pendComp.DesiredState == PolicyState.Disabled;
                        compPend = compCfg;
                    }
                }
                catch { }

                UserConfigured = userCfg;
                ComputerConfigured = compCfg;
                UserEnabled = userEn;
                UserDisabled = userDis;
                ComputerEnabled = compEn;
                ComputerDisabled = compDis;

                UserGlyph = userEn ? "\uE73E" : (userDis ? "\uE711" : string.Empty);
                ComputerGlyph = compEn ? "\uE73E" : (compDis ? "\uE711" : string.Empty);

                Brush? apply2 = apply;
                Brush? danger2 = danger;
