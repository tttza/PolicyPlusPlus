...
{
    // Scopes ADMX-limiting environment variables and enforces timeout.
    private static async Task WithLimitedAdmxAsync(Func<Task> body)
    {
        var prevOnly = Environment.GetEnvironmentVariable("POLICYPLUS_CACHE_ONLY_FILES");
        var prevMax = Environment.GetEnvironmentVariable("POLICYPLUS_CACHE_MAX_POLICIES");
        ...
        try
        {
            await RunWithTimeoutAsync(body).ConfigureAwait(false);
        }
        finally
        {
            Environment.SetEnvironmentVariable("POLICYPLUS_CACHE_ONLY_FILES", prevOnly);
            Environment.SetEnvironmentVariable("POLICYPLUS_CACHE_MAX_POLICIES", prevMax);
        }
    }
    ...
    {
        await WithLimitedAdmxAsync(async () =>
        {
    ...
    public async Task Changing_SourceRoot_Reindexes_And_Affects_HitCounts()
    {
        await WithLimitedAdmxAsync(async () =>
        {
            ...
            try
            {
                ...
                // Precision adjustments in SearchAsync may suppress description-only matches more aggressively
                // when the root contains many assets. Allow a modest increase (<=2x) in reduced asset set to avoid
                // false failures while still detecting pathological growth.
                Assert.True(
    ...
    public async Task SearchFields_Toggles_Work_For_Name_Id_Registry_And_Description()
    {
        await WithLimitedAdmxAsync(async () =>
        {
            ...
            Assert.Contains(
                byIdExact,
                h => string.Equals(h.UniqueId, first.UniqueId, StringComparison.OrdinalIgnoreCase)
            );
