...
namespace PolicyPlusModTests.WinUI3
{
    public class PolicyListRowProjectionTests
    {
        ...
        {
            var userPol = MakeBothPolicy("USER:Sample"); userPol.RawPolicy.Section = AdmxPolicySection.User;
            var compPol = MakeBothPolicy("MACHINE:Sample"); compPol.RawPolicy.Section = AdmxPolicySection.Machine;

            // Pending: user Disabled, computer Enabled
            PendingChangesService.Instance.Pending.Clear();
            PendingChangesService.Instance.Add(new PendingChange { PolicyId = userPol.UniqueID, Scope = "User", DesiredState = PolicyState.Disabled });
            ...
            var row = PolicyListRow.FromGroup(compPol, new[] { userPol, compPol }, new PolFile(), new PolFile());
