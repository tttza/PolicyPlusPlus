...
namespace PolicyPlusModTests.WinUI3.QuickEdit
{
    public class QuickEditRowMultipleListTests
    {
        ...
        public void ReplaceList_ByElementId_UpdatesSecondListAndQueuesPending()
        {
            ...
            var simple = row.OptionElements.First(e =>
                e.Type == OptionElementType.List && !e.ProvidesNames && e.Id == "ListValues"
            );
            var named = row.OptionElements.First(e =>
                e.Type == OptionElementType.List && e.ProvidesNames && e.Id == "NamedList"
            );
            ...
            var pending = PendingChangesService.Instance.Pending.FirstOrDefault(p =>
                p.PolicyId == pol.UniqueID && p.Scope == "User"
            );
