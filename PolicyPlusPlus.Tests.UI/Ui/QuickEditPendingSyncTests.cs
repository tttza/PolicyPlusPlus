...
public class QuickEditPendingSyncTests : IClassFixture<TestAppFixture>
{
    ...
    private static bool UnsavedIndicatorVisible(AutomationElement window)
    {
        try
        {
            ...
            if (unsaved == null) return false;
            return !unsaved.Properties.IsOffscreen.Value;
        }
        catch { return false; }
    }

    private AutomationElement OpenEditWindowRobust(AutomationElement desktop, AutomationElement list, AutomationElement row)
    {
        var end = DateTime.UtcNow.AddMilliseconds(TimeoutMs);
        ...
        for (int attempt = 0; attempt < 6 && DateTime.UtcNow < end; attempt++)
        {
            ...
            if (attempt == 0) InvokePattern(row); // first: invoke
            else if (attempt % 2 == 1) { try { row.DoubleClick(); } catch { } }
            else SendEnter();
    ...
    public void SaveFromMainWindow_QuickEditAndPendingReflectApplied()
    {
        ...
        Assert.True(Retry.WhileFalse(() => UnsavedIndicatorVisible(main), timeout: TimeSpan.FromMilliseconds(5000), interval: TimeSpan.FromMilliseconds(150)).Success);

        var unsaved = main.FindFirstDescendant(cf => cf.ByAutomationId("UnsavedIndicator"));
        Assert.NotNull(unsaved);
        try { unsaved.Click(); } catch { InvokePattern(unsaved); }

        AutomationElement pendingWin = null!;
        var end = DateTime.UtcNow.AddMilliseconds(TimeoutMs);
        while (DateTime.UtcNow < end)
        {
            var maybePending = desktop.FindAllChildren().FirstOrDefault(c => (c.Name ?? string.Empty).Contains("Pending changes", StringComparison.OrdinalIgnoreCase));
            if (maybePending != null) { pendingWin = maybePending; break; }
            Thread.Sleep(180);
        }
        ...
        Assert.True(Retry.WhileTrue(() => UnsavedIndicatorVisible(main), timeout: TimeSpan.FromMilliseconds(6000), interval: TimeSpan.FromMilliseconds(200)).Success, "Unsaved indicator still visible after save.");
        ...
        bool PendingListCleared()
        {
            try
            {
                ...
                if (listEl == null) return false;
                var items = listEl.FindAllDescendants(cf => cf.ByControlType(ControlType.DataItem));
                return items.Length == 0;
            }
            catch { return false; }
        }
        Assert.True(Retry.WhileTrue(() => !PendingListCleared(), timeout: TimeSpan.FromMilliseconds(6000), interval: TimeSpan.FromMilliseconds(250)).Success, "Pending list not cleared after save.");
    }
}
