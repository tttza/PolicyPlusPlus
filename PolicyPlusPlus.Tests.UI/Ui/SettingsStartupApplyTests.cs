...
public class SettingsStartupApplyTests
{
    ...
    private static AutomationElement WaitFor(AutomationElement root, string name, ControlType? type = null, int timeoutMs = 4000)
    {
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "PolicyPlusUITest_PreSeed_" + System.Guid.NewGuid().ToString("N"));
    ...
    private static void OpenTopMenu(AutomationElement window, string title)
    {
        ...
        try { item.Patterns.Invoke.Pattern.Invoke(); } catch { item.Click(); }
        Thread.Sleep(150);
    }
}
