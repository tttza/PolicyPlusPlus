...
public class SettingsPersistenceTests : IClassFixture<TestAppFixture>
{
    ...
    private AutomationElement? GlobalFindByName(string name)
    {
        try
        {
            ...
            return desktop
                .FindAllDescendants()
                .FirstOrDefault(e => (e.Name ?? string.Empty).ToLowerInvariant().Contains(lowered));
        }
        catch
        {
            return null;
            ...
            if (!expanded)
            {
                ...
                catch
                {
                    throw new InvalidOperationException(
                        "Element not invokable: " + (el.Name ?? el.AutomationId)
                    );
                }
            }
        }
    ...
    private static void WaitForSettingsWrite(
        string dir,
    ...
    private static string ReadSettingsJsonFast(string dir)
    {
        var p = Path.Combine(dir, "settings.json");
        var start = Environment.TickCount;
        while (!File.Exists(p) && Environment.TickCount - start < 1500)
            Thread.Sleep(30);
    ...
    {
        if (string.Equals(_openTopMenu, title, StringComparison.OrdinalIgnoreCase))
            return;
        AutomationElement item;
        try
        {
            item = WaitForElementByName(window, title, ControlType.MenuItem);
        }
        catch
        {
            item = GlobalFindByName(title) ?? WaitForElementByName(window, title, null);
        }
        Invoke(item);
        _openTopMenu = title;
        Thread.Sleep(60);
    ...
    private AutomationElement EnsureSubMenu(string parentMenuTitle, string subItemName)
    {
        ...
        try
        {
            return WaitForElementByName(
                _fixture.Host.MainWindow!,
                subItemName,
    ...
    private void TryCloseCustomPolDialog()
    {
        try
        {
            ...
            var okBtn =
                dlg.FindFirstDescendant(cf => cf.ByName("OK"))
                ?? dlg.FindFirstDescendant(cf => cf.ByControlType(ControlType.Button));
            if (okBtn != null)
            {
                Invoke(okBtn);
    ...
    public void ToggleSettings_PersistWrites()
    {
        ...
        var hideItem = WaitForElementByName(
            window,
        ...
        try
        {
            useCustomPol = WaitForElementByName(window, "Use custom .pol", null, timeoutMs: 600);
        }
        catch { }
        ...
        var detailsItem = WaitForElementByName(
            window,
            "Details pane",
            null,
            timeoutMs: PopupTimeoutMs
        ...
        if (!IsChecked(targetThemeEl))
        {
            Toggle(targetThemeEl);
            WaitForSettingsWrite(_fixture.Host.TestDataDirectory, ref writeTime);
        }
        ...
        bool? configuredOnly =
            doc.RootElement.TryGetProperty("ConfiguredOnly", out var coProp)
            && coProp.ValueKind == JsonValueKind.True
                ? true
                : (coProp.ValueKind == JsonValueKind.False ? false : null);
        bool? bookmarksOnly =
            doc.RootElement.TryGetProperty("BookmarksOnly", out var boProp)
            && boProp.ValueKind == JsonValueKind.True
                ? true
                : (boProp.ValueKind == JsonValueKind.False ? false : null);
        bool? hideEmpty =
            doc.RootElement.TryGetProperty("HideEmptyCategories", out var hProp)
            && hProp.ValueKind == JsonValueKind.True
                ? true
                : (hProp.ValueKind == JsonValueKind.False ? false : null);
        bool? showDetails =
            doc.RootElement.TryGetProperty("ShowDetails", out var sdProp)
            && sdProp.ValueKind == JsonValueKind.True
                ? true
                : (sdProp.ValueKind == JsonValueKind.False ? false : null);
        string? theme =
            doc.RootElement.TryGetProperty("Theme", out var tProp)
            && tProp.ValueKind == JsonValueKind.String
                ? tProp.GetString()
                : null;
        string? scale =
            doc.RootElement.TryGetProperty("UIScale", out var sProp)
        ...
        if (
            doc.RootElement.TryGetProperty("CustomPol", out var cpProp)
            && cpProp.ValueKind == JsonValueKind.Object
        )
        {
            if (cpProp.TryGetProperty("Active", out var actProp))
            {
                if (actProp.ValueKind == JsonValueKind.True)
                    customPolActive = true;
                else if (actProp.ValueKind == JsonValueKind.False)
                    customPolActive = false;
            }
        }
