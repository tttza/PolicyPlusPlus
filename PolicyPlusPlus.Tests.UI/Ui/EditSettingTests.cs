...
public class EditSettingTests : IClassFixture<TestAppFixture>
{
    ...
    private static AutomationElement FindByAutomationId(AutomationElement root, string id)
        => Retry.WhileNull(() => root.FindFirstDescendant(cf => cf.ByAutomationId(id)), timeout: TimeSpan.FromMilliseconds(TimeoutMs), interval: TimeSpan.FromMilliseconds(PollMs)).Result
    ...
    { try { el.Patterns.Invoke.Pattern.Invoke(); } catch { } }

    private static void SelectRow(AutomationElement row)
    { try { row.Patterns.SelectionItem.Pattern.Select(); } catch { } }
    ...
    private static bool AnyPendingShowing(AutomationElement window)
    {
        try
        {
            ...
            if (unsaved == null) return false; return !unsaved.Properties.IsOffscreen.Value;
        }
        catch { return false; }
    }
    ...
    private static AutomationElement WaitEditWindow(AutomationElement desktop)
    {
        ...
        while (DateTime.UtcNow < end)
        {
            editWin = desktop.FindAllChildren().FirstOrDefault(c => (c.Name ?? string.Empty).Contains("Edit Policy Setting", StringComparison.OrdinalIgnoreCase));
            if (editWin != null) return editWin;
            Thread.Sleep(PollMs);
        }
        throw new TimeoutException("Edit window not found");
    }

    private static bool TryFindEditWindowQuick(AutomationElement desktop, out AutomationElement? editWin)
    {
        editWin = desktop.FindAllChildren().FirstOrDefault(c => (c.Name ?? string.Empty).Contains("Edit Policy Setting", StringComparison.OrdinalIgnoreCase));
        return editWin != null;
    }
    ...
    private static void RobustOpen(AutomationElement mainWindow, AutomationElement desktop, AutomationElement policyList, AutomationElement row)
    {
        // Ensure list has focus first
        try { policyList.Focus(); } catch { }
        Thread.Sleep(120);
        SelectRow(row);
        Thread.Sleep(80);
        ...
        {
            if (TryFindEditWindowQuick(desktop, out _)) return; // already open
            // Press Enter
            SendEnter();
            Thread.Sleep(200 + attempt * 60);
            if (TryFindEditWindowQuick(desktop, out _)) return;
            // Invoke row directly
            InvokePattern(row);
    ...
    private static bool GetRadioChecked(AutomationElement? el)
    {
        ...
        try { return el.Patterns.SelectionItem.Pattern.IsSelected.Value; } catch { }
        try { var tp = el.Patterns.Toggle.Pattern; return tp.ToggleState.Value == FlaUI.Core.Definitions.ToggleState.On; } catch { }
        return false;
    }
    ...
    private static void EnsureRadioSelected(AutomationElement radio)
    {
        ...
        while (DateTime.UtcNow < end)
        {
            if (GetRadioChecked(radio)) return;
            attempt++;
            InvokePattern(radio);
            try { radio.Patterns.SelectionItem.Pattern.Select(); } catch { }
            try { radio.Patterns.Toggle.Pattern.Toggle(); } catch { }
            Thread.Sleep(140 + attempt * 50);
        }
        if (!GetRadioChecked(radio)) throw new TimeoutException("Radio not selected: " + (radio.Name ?? "(unnamed)"));
    }
    ...
    public void EditBooleanPolicy_EnableAndSave_PendingClearedAfterSave()
    {
        ...
        var okBtn = editWin.FindFirstDescendant(cf => cf.ByName("OK")) ?? editWin.FindFirstDescendant(cf => cf.ByControlType(ControlType.Button).And(cf.ByName("OK")));
        Assert.NotNull(okBtn);
        InvokePattern(okBtn);
        Thread.Sleep(400);

        Assert.True(Retry.WhileFalse(() => AnyPendingShowing(window), timeout: TimeSpan.FromMilliseconds(4000), interval: TimeSpan.FromMilliseconds(150)).Success);

        // Save (Ctrl+S)
        Keyboard.Press(VirtualKeyShort.CONTROL); Keyboard.Press(VirtualKeyShort.KEY_S); Keyboard.Release(VirtualKeyShort.KEY_S); Keyboard.Release(VirtualKeyShort.CONTROL);
        Thread.Sleep(650);

        Assert.True(Retry.WhileTrue(() => AnyPendingShowing(window), timeout: TimeSpan.FromMilliseconds(5000), interval: TimeSpan.FromMilliseconds(150)).Success, "Pending indicator still visible after save.");
        ...
        if (!GetRadioChecked(enabledRadio2)) EnsureRadioSelected(enabledRadio2);
        ...
        if (cancelBtn != null) InvokePattern(cancelBtn);
    }
}
