    ...
    public static string ResolveAdmxAssetsPath()
    {
        ...
        foreach (var c in localCandidates)
        {
            try
            {
                if (
                    Directory.Exists(c)
                    && Directory.EnumerateFiles(c, "*.admx", SearchOption.TopDirectoryOnly).Any()
                )
                    return c;
            }
            catch { }
        ...
        try
        {
            ...
            for (int i = 0; i < 12 && dirInfo != null; i++)
            {
                ...
                if (Directory.Exists(shared) && Directory.EnumerateFiles(shared, "*.admx").Any())
                    return shared;
                var legacy = Path.Combine(
                    dirInfo.FullName,
                    "PolicyPlusPlus.Tests.UI",
                    "TestAssets",
                    "Admx"
                );
                if (Directory.Exists(legacy) && Directory.EnumerateFiles(legacy, "*.admx").Any())
                    return legacy;
        ...
        throw new DirectoryNotFoundException(
            "ADMX test assets folder not found from base directory: " + baseDir
        );
    }
